...
public class HalsteadMetrics {
    ...
    public HalsteadMetrics() throws IOException {
        DistOperators=0;
        DistOperands=0;
        TotOperators=0;
        TotOperands=0;

        String[] HALSTEAD_METRICS = {"Vocabulary","Program Length","Calculated Length","Volume","Difficulty","Effort"};
        File tasks = new File("./halstead.csv");
        FileWriter outputfile = new FileWriter(tasks);
        outputfile.write("class,");
        outputfile.write(String.join(",",HALSTEAD_METRICS));
        outputfile.write("\n");
        outputfile.close();
    }
    ...
    public void setParameters(int DistOprt, int DistOper, int TotOprt, int TotOper, String fileName)
    {
    ...
    public void printToCSV() throws IOException {
        FileWriter outputfile = new FileWriter("./halstead.csv",true);

        String toPrint = String.format("%s,%d,%d,%.4f,%.4f,%.4f,%.4f",name,getVocabulary(),getProglen(),getCalcProgLen(),
                getVolume(),getDifficulty(),getEffort());

        outputfile.append(toPrint);
        outputfile.append("\n");
        outputfile.close();
    }
}
